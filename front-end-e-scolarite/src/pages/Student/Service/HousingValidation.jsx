...
const HousingValidation = () => {
  ...
  const handleDecision = async (id, decision, chambreCode, occupantsActuels) => {
    // RÈGLE MÉTIER : Blocage préventif côté Front si l'admin essaie de forcer une chambre pleine
    if (decision === 'VALIDE' && occupantsActuels >= 4) {
      setNotification({ 
        type: 'error', 
        message: `Action refusée : La chambre ${chambreCode} contient déjà 4 personnes !` 
      });
      setTimeout(() => setNotification(null), 4000);
      return;
    }

    if (!window.confirm(`Confirmer cette décision (${decision}) pour le logement ?`)) return;

    setActionLoading(id);
    try {
      // ✅ URL corrigée : logements/{id}/decider/ (sans "demandes/")
      await apiClient.post(`logements/${id}/decider/`, { statut: decision });
      setNotification({ 
        type: 'success', 
        message: decision === 'VALIDE' ? 'Logement approuvé avec succès !' : 'Demande déclinée.' 
      });
      fetchRequests(); // Rafraîchir la liste et les compteurs d'occupants
    } catch (error) {
      setNotification({ 
        type: 'error', 
